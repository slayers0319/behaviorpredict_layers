...
#include <behaviorpredict_layers/behaviorpredict_layer.h>
...
BehaviorPredictLayer::BehaviorPredictLayer() {}
...
void dataSplit(const std_msgs::String::ConstPtr& msg){ 
    ...
    ROS_INFO("none");
        ...
        ROS_INFO("clear behavior predict layers");
        return;
    }
...
void BehaviorPredictLayer::onInitialize(){ 
#if DEBUG
    ROS_INFO("behaviorpredict_layers in DEBUG mode!!");
#endif
    ...
    dynamic_reconfigure::Server<costmap_2d::GenericPluginConfig>::CallbackType cb = boost::bind(&BehaviorPredictLayer::reconfigureCB, this, _1, _2);
...
void BehaviorPredictLayer::reconfigureCB(costmap_2d::GenericPluginConfig &config, uint32_t level)
{
    enabled_ = config.enabled;
}

void BehaviorPredictLayer::updateBounds(double robot_x, double robot_y, double robot_yaw, double* min_x, double* min_y, double* max_x, double* max_y){
...
void BehaviorPredictLayer::updateCosts(costmap_2d::Costmap2D& master_grid, int min_i, int min_j, int max_i, int max_j){
    if (!enabled_)
        return;
...
void BehaviorPredictLayer::rasterizePolygon(const std::vector<PointInt>& polygon, std::vector<PointInt>& polygon_cells){
...
void BehaviorPredictLayer::polygonOutlineCells(const std::vector<PointInt>& polygon, std::vector<PointInt>& polygon_cells){
...
void BehaviorPredictLayer::raytrace(int x0, int y0, int x1, int y1, std::vector<PointInt>& cells){
    int dx = abs(x1 - x0);
    int dy = abs(y1 - y0);
    PointInt pt;
    pt.x = x0;
